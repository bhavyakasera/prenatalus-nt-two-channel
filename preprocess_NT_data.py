...
DATA_FOLDER = '/Users/bhavyakasera/Documents/PrenatalUS/cleaned_and_cropped'
IMAGE_FOLDER = '/Users/bhavyakasera/Documents/PrenatalUS/data/prenatalUS_NT'

#
# def train_val_split(samples):
#     return np.split(np.random.choice(samples, len(samples), replace=False), [int(0.75*len(samples))])
#
#
# def spacer():
#     print('-' * 50)
...
#             if image_extension.lower() in ['.jpg', '.tif', '.jpeg', '.tiff']:
#                 approved_ext = True
...
train_patients_list = os.listdir(f'{IMAGE_FOLDER}_train/images')
train_patients = [f'Patient {patient_num.split("_")[0]}' for patient_num in train_patients_list]
val_patients_list = os.listdir(f'{IMAGE_FOLDER}_val/images')
val_patients = [f'Patient {patient_num.split("_")[0]}' for patient_num in val_patients_list]
print('num train = '+str(len(train_patients)), ', num val = '+str(len(val_patients)))
...
for patient_name in train_patients:
    ...
    img_save = f'{IMAGE_FOLDER}_train/images/{patient}_3.jpg'
    ...
    baby_save = f'{IMAGE_FOLDER}_train/masks/{patient}_3_baby_mask.jpg'
    ...
    NT_save = f'{IMAGE_FOLDER}_train/masks/{patient}_3_nt_mask.jpg'
    ...
    anomaly_save = f'{IMAGE_FOLDER}_train/masks/{patient}_3_anomaly_mask.jpg'
    cv2.imwrite(anomaly_save, cv2.convertScaleAbs(anomaly_mask, alpha=(255.0)))
    print(f'saved patient {patient} in train')

for patient_name in val_patients:
    ...
    us_img = np.array((Image.open(cropped_image)).getchannel("R"))
    us_img = st.resize(us_img, us_img.shape)
    img_save = f'{IMAGE_FOLDER}_val/images/{patient}_3.jpg'
    cv2.imwrite(img_save, cv2.convertScaleAbs(us_img, alpha=(255.0)))
    ...
    baby_mask = get_mask(baby_path)
    baby_mask[baby_mask > 0] = 255.0
    baby_save = f'{IMAGE_FOLDER}_val/masks/{patient}_3_baby_mask.jpg'
    cv2.imwrite(baby_save, cv2.convertScaleAbs(baby_mask, alpha=(255.0)))
    ...
    NT_mask = get_mask(NT_path, True)
    NT_mask[NT_mask > 0] = 255.0
    NT_save = f'{IMAGE_FOLDER}_val/masks/{patient}_3_nt_mask.jpg'
    cv2.imwrite(NT_save, cv2.convertScaleAbs(NT_mask, alpha=(255.0)))
    ...
    anomaly_save = f'{IMAGE_FOLDER}_val/masks/{patient}_3_anomaly_mask.jpg'
    cv2.imwrite(anomaly_save, cv2.convertScaleAbs(anomaly_mask, alpha=(255.0)))
    print(f'saved patient {patient} in val')